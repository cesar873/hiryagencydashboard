...
export function findInvoicingHeader(rows) {
  for (let i = 0; i < Math.min(rows.length, 60); i++) {
    ...
    const hasApproved = row.some(c => /^approved(\s+\w+)?$/.test(c));
...
export function parseBookkeepingRows(rows) {
  ...
  // 2) Header detection for E:L. Column M is always `account` regardless of header.
  //    Look in row 1 first; if it doesn't classify, fall back to typical positions
  //    (E=Date, F=Vendor, G=Description, H=Amount).
  const headerRow = (rows[0] || []).map(c => String(c || '').trim());
  const classified = {};                  // { canonicalKey: colIdx }
  const rawCols = [];                     // [{ colIdx, label }] for unclassified E–L
  for (let c = 4; c <= 11; c++) {         // E=4, L=11
    const label = headerRow[c] || '';
    const key = classifyBookkeepingHeader(label);
    if (key && classified[key] == null) {
      classified[key] = c;
    } else if (label) {
      rawCols.push({ colIdx: c, label });
    }
  }
  // Sensible positional fallbacks if headers were blank
  if (classified.date == null)        classified.date = 4;        // E
  if (classified.vendor == null)      classified.vendor = 5;      // F
  if (classified.description == null) classified.description = 6; // G
  if (classified.amount == null)      classified.amount = 7;      // H

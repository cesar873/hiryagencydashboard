...
// Matchers test for the two words INDEPENDENTLY (order-free) and accept any
// of Invoice / Invoices / Invoicing — so "Invoices (Clients)",
// "Invoicing (Client)", "Client Invoices", etc. all match. Receivable is
// kept as a legacy fallback name.
const INVOICE_WORD = /invoic/i;   // matches invoice, invoices, invoicing
export function matchInvoicingCandidateTab(tabs) {
  return tabs.find(t => INVOICE_WORD.test(t) && /candidate/i.test(t)) ||
         tabs.find(t => /candidate/i.test(t) && /receivable/i.test(t)) ||
         null;
}
export function matchInvoicingClientTab(tabs) {
  return tabs.find(t => INVOICE_WORD.test(t) && /client/i.test(t)) ||
         tabs.find(t => /client/i.test(t) && /receivable/i.test(t)) ||
         null;
}
...
// Tab names rarely change, and listing them is a full spreadsheets.get
// round-trip. Cache the listing per warm Lambda instance for a short TTL so
// back-to-back writes (approve / status / category / comment) don't each pay
// that latency — a meaningful chunk of the "stuck on saving" slowness.
let _tabCache = null;
let _tabCacheAt = 0;
const TAB_CACHE_TTL_MS = 60 * 1000;
async function listTabs(sheets) {
  const now = Date.now();
  if (_tabCache && (now - _tabCacheAt) < TAB_CACHE_TTL_MS) return _tabCache;
  const meta = await sheets.spreadsheets.get({ spreadsheetId: SHEET_ID });
  _tabCache = meta.data.sheets.map(s => s.properties.title);
  _tabCacheAt = now;
  return _tabCache;
}
...
function classifyBookkeepingHeader(label) {
  ...
  if (/vendor|merchant|payee|name|question/.test(s)) return 'vendor';
...
export function classifyBookkeepingColumns(headerRowRaw) {
  ...
  // The actual sheet header text for each classified column, so the UI can
  // display the real title (e.g. a vendor column renamed "Question" shows as
  // "Question", not the hardcoded "Vendor"). Falls back to a sensible default
  // when the header cell is blank.
  const labels = {
    date:        headerRow[cols.date]        || 'Date',
    vendor:      headerRow[cols.vendor]      || 'Vendor',
    description: headerRow[cols.description] || 'Description',
    amount:      headerRow[cols.amount]      || 'Amount',
    account:     headerRow[cols.account]     || 'Account'
  };
  return { cols, rawCols, labels };
}
...
export function parseBookkeepingRows(rows) {
  ...
  const { cols: classified, rawCols, labels } = classifyBookkeepingColumns(rows[0]);
  ...
  return { coa, transactions, labels };
}
